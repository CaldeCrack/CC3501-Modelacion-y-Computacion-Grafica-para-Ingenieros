...
P0 = np.array([[0, -5, 0]]).T
P1 = np.array([[6.7, -5, 0]]).T
P2 = np.array([[6.7, 5, 0]]).T
P3 = np.array([[0, 5, 0]]).T
...
P4 = np.array([[0, 5, 0]]).T
P5 = np.array([[-6.7, 5, 0]]).T
P6 = np.array([[-6.7, -5, 0]]).T
P7 = np.array([[0, -5, 0]]).T
...
# # Creamos la curva
GMh = hermiteMatrix(P1, P2, T1, T2)
HermiteCurve4 = evalCurve(GMh, N)

HermiteCurveFinal = np.concatenate((HermiteCurve1, HermiteCurve2, HermiteCurve3, HermiteCurve4), axis=0)
...
def on_draw():
    ...
    transformRainbow = tr.matmul([tr.translate(BezierCurve[controller.step, 0], BezierCurve[controller.step, 1], BezierCurve[controller.step, 2])])
