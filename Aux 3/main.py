...
def main():
    ...
    gpuFloor.texture = textureSimpleSetup("assets/dirt.png", GL_REPEAT, GL_REPEAT, GL_NEAREST, GL_NEAREST)

    gpuDirt = GPUShape().initBuffers()
    texPipeline.setupVAO(gpuDirt)
    gpuDirt.fillBuffers(cubeShape.vertices, cubeShape.indices)
    gpuDirt.texture = textureSimpleSetup("assets/dirt.png", GL_REPEAT, GL_REPEAT, GL_NEAREST, GL_NEAREST)

    gpuShelf = GPUShape().initBuffers()
    texPipeline.setupVAO(gpuShelf)
    gpuShelf.fillBuffers(cubeShape.vertices, cubeShape.indices)
    gpuShelf.texture = textureSimpleSetup("assets/bookshelf.png", GL_REPEAT, GL_REPEAT, GL_NEAREST, GL_NEAREST)

    gpuStone = GPUShape().initBuffers()
    texPipeline.setupVAO(gpuStone)
    gpuStone.fillBuffers(cubeShape.vertices, cubeShape.indices)
    gpuStone.texture = textureSimpleSetup("assets/cobblestone.png", GL_REPEAT, GL_REPEAT, GL_NEAREST, GL_NEAREST)
